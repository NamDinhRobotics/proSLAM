...
namespace proslam {
  ...
  public:
    ...
    inline void setContext(const LocalMap* context_) {_context = context_;}

  protected:

    Landmark* _landmark       = 0;    //ds must be set at construction
    const LocalMap* _context  = 0;    //ds every item has NO CONTEXT upon creation and EXACTLY ONE CONTEXT once claimed
