...
int32_t main(int32_t argc, char ** argv) {
  ...
  relocalizer->aligner()->setMaximumErrorKernel(0.25);
...
//  std::string topic_image_stereo_right     = "/camera_right/image_raw";
  std::string topic_image_stereo_left      = "/cam0/image_raw";
  std::string topic_image_stereo_right     = "/cam1/image_raw";
  ...
  tracker->gridSensor()->setTargetNumberOfPoints(500);
  tracker->gridSensor()->setDetectorThresholdMinimum(1);
  ...
  if (use_gui) {
    ...
    context_viewer = new TrackingContextViewer(world_context, 0.05);
  ...
  while ((base_message = sensor_message_reader.readMessage()) && running) {
    ...
    if (synchronizer.messagesReady()) {
      ...
      if (world_context->currentTrackingContext()->frames().size() == 0 && message_image_left->hasOdom()) {
        ...
        //ds set initial position to odometry start
