...
AlignerViewer* aligner_viewer         = 0;
...
//ds projection matrix loading (maximum precision)
...
int32_t main(int32_t argc, char ** argv) {
  ...
  //ds open calibration file TODO remove (maximum precision)
  ProjectionMatrix projection_matrix_camera_left;
  ProjectionMatrix projection_matrix_camera_right;
  loadCalibrationKITTI("calib.txt", projection_matrix_camera_left, projection_matrix_camera_right);
  ...
  while ((base_message = sensor_message_reader.readMessage())) {
    ...
    if (sensor_msg->topic() == topic_image_stereo_left) {
      ...
      Camera* camera_right = new Camera(message_image_right->image().rows,
                                        ...
                                        false,
                                        projection_matrix_camera_right);
  ...
  if (use_gui) {
    ...
    context_viewer = new TrackingContextViewer(world_context, 1.0);
...
//      if (use_gui) {
//
//        //ds update alignment viewer
//        aligner_viewer->setPointsQuery(tracker->aligmentPointsQuery());
//        aligner_viewer->setPointsReference(tracker->aligmentPointsReference());
//        aligner_viewer->setTransformQueryToReferenceGuess(odometry_robot_to_world_previous.inverse());
//        aligner_viewer->setTransformQueryToReference(tracker->alignmentTransformQueryToReference());
//      }
      ...
      if (message_image_left->hasOdom()) {
        ...
        if (world_context->currentTrackingContext()->frames().size() > 1) {
          const TransformMatrix3D world_previous_to_current_ground_truth = robot_to_world_ground_truth*odometry_robot_to_world_previous_ground_truth.inverse();
          errors_translation_relative.push_back((world_previous_to_current.translation()-world_previous_to_current_ground_truth.translation()).norm());
        }
        squared_errors_translation_absolute.push_back((tracker->robotToWorld().translation()-robot_to_world_ground_truth.translation()).squaredNorm());

        tracker->setOdometryRobotToWorld(robot_to_world_ground_truth);
        odometry_robot_to_world_previous_ground_truth = robot_to_world_ground_truth;
      } else {
...
//        aligner_viewer->updateGL();
        ui_server->processEvents();
      }

//      if (number_of_processed_frames_total == 100) {
//        running = false;
//      }
    }
  }
